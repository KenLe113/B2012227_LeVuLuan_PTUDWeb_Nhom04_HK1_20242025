<template>
  <aside
    ...
    id="sidenav-main" v-if="login">
    <div class="sidenav-header">
      <i class="fas fa-times p-3 cursor-pointer text-white opacity-5 position-absolute end-0 top-0 d-none d-xl-none"
        aria-hidden="true" id="iconSidenav"></i>
      <a class="navbar-brand m-0" href="#" target="_blank">
        <span class="ms-1 font-weight-bold" style="color: aqua">LBOOK SHOP </span>
      </a>
    </div>
    ...
    <div class="collapse navbar-collapse w-auto" id="sidenav-collapse-main">
      <ul class="navbar-nav">
        <li class="nav-item">
          <router-link v-bind:class="'nav-link text-white ' +
            ($route.path == '/' ? 'bg-gradient-primary active' : '')
            " to="/">
        ...
        <li class="nav-item">
          <router-link v-bind:class="'nav-link text-white ' +
            ($route.path == '/addProduct' ? 'bg-gradient-primary active' : '')
            " to="/addProduct">
            <div class="text-white text-center me-2 d-flex align-items-center justify-content-center">
        ...
        <li class="nav-item">
          <router-link v-bind:class="'nav-link text-white ' +
            ($route.path == '/allOrders' ? 'bg-gradient-primary active' : '')
            " to="/allOrders">
            <div class="text-white text-center me-2 d-flex align-items-center justify-content-center">
              <i class="material-icons opacity-10">list</i>
            </div>
            <span class="nav-link-text ms-1">Danh sách khách mượn</span>
            <span v-if="unReadOrderNotifications > 0" v-text="'(' + unReadOrderNotifications + ')'"
    ...
    getAdmin: async function () {
      ...
      if (localStorage.getItem(this.$accessTokenKey)) {
        ...
        if (response.data.status == 'success') {
          ...
          store.commit(
            'setUnReadOrderNotifications',
            response.data.unReadOrderNotifications
          );
          ...
          if (typeof global.socketIO !== 'undefined') {
            ...
            global.socketIO.on('newOrder', function () {
              let tempUnReadOrderNotifications =
                store.getters.getUnReadOrderNotifications;
